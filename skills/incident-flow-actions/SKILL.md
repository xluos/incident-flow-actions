...
botmux incident-flow-actions:send \
  --incident-id '<稳定任务ID>' \
  --summary-file '<本次群回复正文绝对路径>' \
  --owner-file '<owner-identity.json>'
```

默认用 `--summary-file` 或 `--summary` 生成 schema 2.0 基础卡。正文按当前用户的问题重新写成简短自然的回复；旧报告用于复用证据，历史卡片不直接重发。只有需要特殊布局时才用 `--card-file`，并生成本次正文。正文、按钮和 @ 在同一张卡中发送。重复传 `--action <动作ID>` 可从当前方案选择本次适用的按钮；不传则保持方案原有按钮。不要显示不满足授权条件的动作。`--profile <方案名>` 选择已配置方案；`--dry-run` 只生成预览，不发送消息。命令必须在有已验证请求人的 Botmux 会话中运行，卡片以当前机器人的身份发送。
...
正文沿用当前角色的沟通要求，用 `haohao-shuohua` 的群回复方式润色，排查链路和工程清单放在报告链接里。发送失败先确认错误及是否已有消息 ID，只修正失败项后重试同一份结果；不另发“结果在下一张卡片”等重复通知。成功后回读卡片并保存消息 ID。
